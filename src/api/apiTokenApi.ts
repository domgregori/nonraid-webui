...
export const apiTokenApi = {
  ...
  // adding a trusted SSH key is (see routes/auth.ts).
  create: (name: string, currentPassword: string, totpCode?: string) =>
    request<CreatedApiToken>('/api/auth/tokens', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ name, currentPassword, totpCode }),
    }),
