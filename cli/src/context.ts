...
// dependency on a config file existing at all), then the saved config from `nwctl login`.
...
export async function resolveClient(): Promise<ApiClient> {
  ...
  if (!host || !token) {
    console.error('Not logged in. Run `nwctl login` first (or set NONRAID_HOST / NONRAID_TOKEN).');
    process.exit(1);
  }
