...
/** Puts the drive into standby immediately - hdparm's own documented spin-down command. Manual,
 *  one-shot action - the automatic idle-timeout case lives in hdIdle.ts instead (see its own doc
 *  comment for why hdparm's own ATA standby timer isn't used for that). */
