...
| POST | `/array/import/commit` | `{ token, stopContainers? }` | Re-validates against live disk state, unmounts everything, imports the superblock. `409` if any disk has a size mismatch against it (hard-blocks, no override - this can corrupt filesystems). `400` if the preview token expired (30 min TTL) or was already used. `stopContainers: true` opts into stopping Docker + running LXC containers first if something (e.g. Docker/LXC storage relocated onto the array being replaced) has a file open on a disk; restarted afterward if the commit itself then fails, same as `/array/stop`. |
...
| POST | `/luks/:slot/unlock` | `{ passphrase? }` or `{ keyfileBase64? }` | Not step-up gated. Opens + mounts a locked LUKS data disk - via `passphrase`, an ad-hoc uploaded keyfile's bytes (`keyfileBase64`, piped to cryptsetup over stdin, never written to disk), or the shared stored keyfile if neither is given. `keyfileBase64` takes priority if both are somehow given. |
...
| POST | `/system/reload-driver` | - | Manual retry of the driver reload a superblock restore already attempts automatically. Both this and the automatic reload always stop Docker + running LXC containers first if something is holding an array disk busy (no opt-in flag - there's no admin watching either one to ask), restarting them afterward regardless of outcome. |
| POST | `/system/restart-services` | `{ restartDocker? }` | Restarts SMB, NFS, reloads the driver, optionally restarts Docker, then self-restarts nonraid-webui (drops the connection - the client reconnects automatically). The "make everything take effect" action after a config restore. Same always-on Docker/LXC-stop-and-restart as `/system/reload-driver` around the driver-reload step. That reload only re-imports the superblock - it never mounts array disks - so if Docker's storage is on an array disk and the array isn't started, the separate Docker *restart* below is skipped instead of bouncing it against an unmounted path (reported in the response as `docker: { ok: false, message }` rather than attempted). |
